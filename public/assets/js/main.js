...
let settingSnap = false    // Not used currently
let infoShown = false      // Is infobox displayed
let labelField = 'label'   // Which field to show in labels
let iconPrefix             // Global prefix string appended to all icons
let vscode                 // VS Code API instance, can only be fetched once
let filters = ''           // Resource filter string (comma sep list)
...
//
function exportPNG() {
  const data = cy.png({ scale: 2.0, output: 'base64' })
